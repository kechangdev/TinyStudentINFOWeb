...
//     const char* delete_student_sql = "DELETE FROM student WHERE id = ?;";
...
//
//     // 删除 student 表中的记录
//     rc = sqlite3_prepare_v2(db, delete_student_sql, -1, &stmt, nullptr);
//     if (rc != SQLITE_OK) {
//         // std::cerr << "无法准备删除 student 语句: " << sqlite3_errmsg(db) << std::endl;  // 修改错误信息
//         sqlite3_exec(db, "ROLLBACK;", nullptr, nullptr, nullptr);
//         res.status_code = 500;
//         res.body = "Database wrong";
//         res.headers["Content-Type"] = "text/plain";
...
//     rc = sqlite3_step(stmt);
//     if (rc != SQLITE_DONE) {
//         // std::cerr << "删除 student 记录失败: " << sqlite3_errmsg(db) << std::endl;  // 修改错误信息
//         sqlite3_finalize(stmt);
//         sqlite3_exec(db, "ROLLBACK;", nullptr, nullptr, nullptr);
//         res.status_code = 500;
//         res.body = "Database wrong";
...
HttpResponse Handlers::studentHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(STUDENT_PATH) + ".html");
    return res;
}
...
HttpResponse Handlers::logsHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(LOGS_PATH) + ".html");
    return res;
}
...
HttpResponse Handlers::rootHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(ROOT_PATH) + "login.html");
    return res;
}
...
HttpResponse Handlers::indexHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(INDEX_PATH) + ".html");
    return res;
}
...
HttpResponse Handlers::loginHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(LOGIN_PATH) + ".html");
    return res;
}
...
HttpResponse Handlers::registerHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(REGISTER_PATH) + ".html");
    return res;
}
...
HttpResponse Handlers::adminHandler(const HttpRequest& req) {
    ...
    res.status_code = 200;
    res.headers["Content-Type"] = "html";
    res.body = Utils::readHTML(std::string(WWW_ADDR) + std::string(ADMIN_PATH) + ".html");
    return res;
}
...
HttpResponse Handlers::registerVHandler(const HttpRequest& req) {
    ...
    std::string cookie;
    {
        // 加锁保护 Auth::registerUser(...) 里的数据库访问
        std::lock_guard<std::mutex> lock(DatabaseManager::getInstance().dbMutex);
        cookie = Auth::registerUser(username, password, invite_code);
    }
    if (cookie == "null") {
        res.status_code = 400;
        res.headers["Content-Type"] = "text/plain";
...
HttpResponse Handlers::logOut(const HttpRequest& req) {
    ...
    bool success;
    {
        // 加锁保护 Auth::logoutUser(...) 里的数据库访问
        std::lock_guard<std::mutex> lock(DatabaseManager::getInstance().dbMutex);
        success = Auth::logoutUser(cookie);
    }

    // 无论是否成功，都要清空客户端 cookie
    res.status_code = 200;
    res.headers["Content-Type"] = "text/plain";
    res.headers["Set-Cookie"] = "cookie=; Expires=Thu, 01 Jan 1970 00:00:00 GMT; path=/";
    res.body = success ? "success" : "success";  // 代码里都返回了 "success"
    return res;
}
...
HttpResponse Handlers::isloginHandler(const HttpRequest& req) {
    ...
    {
        std::lock_guard<std::mutex> lock(DatabaseManager::getInstance().dbMutex);
        valid = Auth::validateCookie(cookie);
    }
    if (valid) res.status_code = 200;
    else       res.status_code = 400;
    return res;
}
...
HttpResponse Handlers::isrootHandler(const HttpRequest& req) {
    ...
    {
        std::lock_guard<std::mutex> lock(DatabaseManager::getInstance().dbMutex);
        root = Auth::isRoot(cookie);
    }
    if (root) res.status_code = 200;
    else      res.status_code = 400;
    return res;
}
...
HttpResponse Handlers::getStudentInfo(const HttpRequest& req) {
...

    {
        ...
        const char* sql = R"(
            SELECT users.id, users.username, users.permission_level
            FROM auth JOIN users ON auth.user_id = users.id
            WHERE auth.cookie = ? AND auth.expires_at > strftime('%s','now');
        )";
        sqlite3_stmt* stmt;
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
            res.status_code = 500;
        ...
        int user_id = sqlite3_column_int(stmt, 0);
        // 下面这行并没有被用到 username 变量，留给需要时使用
        // std::string username = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
        int permission_level = sqlite3_column_int(stmt, 2);
        sqlite3_finalize(stmt);
        ...
        if (permission_level == 0) {
            ...
            const char* sql_all = R"(
                SELECT users.id, users.username,
                       student.algorithms_score, student.data_structures_score,
                       student.operating_systems_score, student.databases_score,
                       student.computer_networks_score
                FROM users JOIN student ON users.id = student.id;
            )";
            ...
            // 学生：查看自己
            const char* sql_self = R"(
                SELECT student.algorithms_score, student.data_structures_score,
                       student.operating_systems_score, student.databases_score,
                       student.computer_networks_score
                FROM student WHERE student.id = ?;
            )";
...
HttpResponse Handlers::addStudentInfo(const HttpRequest& req) {
    ...
    int permission_level;
    {
        ...
        const char* sql = R"(
            SELECT users.id, users.permission_level
            FROM auth JOIN users ON auth.user_id = users.id
            WHERE auth.cookie = ? AND auth.expires_at > strftime('%s','now');
        )";
        sqlite3_stmt* stmt;
        ...
        permission_level = sqlite3_column_int(stmt, 1);
        sqlite3_finalize(stmt);
...

    {
        ...
        std::string password = name;
        std::string password_hash = password; // 简单做法: 明文当密码

        sqlite3_bind_int(insert_user_stmt, 1, id);
        sqlite3_bind_text(insert_user_stmt, 2, name.c_str(), -1, SQLITE_STATIC);
        sqlite3_bind_text(insert_user_stmt, 3, password_hash.c_str(), -1, SQLITE_STATIC);
        sqlite3_bind_int(insert_user_stmt, 4, 1);
        ...
        const char* insert_student_sql = R"(
            INSERT INTO student (
                id, algorithms_score, data_structures_score,
                operating_systems_score, databases_score, computer_networks_score
            ) VALUES (?, ?, ?, ?, ?, ?);
        )";
        sqlite3_stmt* insert_student_stmt;
...
HttpResponse Handlers::modifyStudentInfo(const HttpRequest& req) {
...

    {
        ...
        const char* update_sql = R"(
            UPDATE student
            SET algorithms_score = ?, data_structures_score = ?,
                operating_systems_score = ?, databases_score = ?,
                computer_networks_score = ?
            WHERE id = ?;
        )";
